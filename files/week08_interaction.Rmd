...
Use this document to add code and knit to explore options for adding interactive elements to static HTML documents. For more advanced interactivity see **shiny** apps.

This is a tasting menu style session so you can get a "taste" of several **htmlwidgets**-based packages that allow you to add interactivity. Knit the `Rmd` document to explore the interactive plots and data in the HTML output.
...
We'll be loading additional packages as we go so it is clearer which package we are using in each section.
...
We will work with the Palmer penguin data for the first two packages we'll explore.
...
In R, the **plotly** package is for building stand-alone interactive graphics or for making our static plots interactive. `r emo::ji("astonished")`
...
You are welcome to add code to print this plot if you'd like, but we won't discuss it. Instead let's jump to adding interactivity.
...
I found things were a little more awkward for plots with legends. It helps a bit if the legend is removed from one of the plots using `plotly::style()`.
...
You can see you can filter by specific columns. 
...
Note the `data` argument added in `leaflet()` in the code. **leaflet** automatically finds lat/long columns if named something recognizable (e.g., `lng`, `long`, `lat`, `latitude`).
...
Explore the output plot in the HTML document. Hovering over the plot shows the time as well as the data in the line.
...
Explore the range selector in the plot in the HTML document.
...
The tooltip shows data from both time series at once. The labels we created are shown in the tooltip.
