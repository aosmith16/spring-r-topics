...
You can find today's slides [here](https://aosmith16.github.io/spring-r-topics/slides/week05_gt_flair.html#1). The slides contain introductory material as well as a list of resources that aren't included here.
...
We'll choose all columns in `sza50` except the first one using `contains()`. We cannot currently use code like `col1:col3`, although this is now available in the [development version of **gt**](https://github.com/rstudio/gt/issues/611).
...
We could focus on the cell fill instead of text using `cell_fill()` as the style function. Here we'll choose the `rows` of interest using a logical statement. The cell we want is the maximum value in the `population` column.
...
We can highlight a set of values in much the same way. For example, maybe we want to highlight the population values before 2015 as red and italic and 2015 and later as blue.
...
We've been highlighting values within one column, but the process works the same way if we want to highlight across many columns. However, this only allows us to highlight rows for every column we choose the same way.
...
The `text_transform()` function does the heavy lifting in combination with `web_image()` if you provide URL 's to images online or `local_image()` if you are providing paths to local copies of images. See other available **gt** image functions [here](https://gt.rstudio.com/reference/index.html#section-image-addition-functions).
...
First up is a APA theme by [Anthony Schmidt](https://www.anthonyschmidt.co/post/2020-06-03-making-apa-tables-with-gt/). Additional options to `tab_options()` is not included here since it doesn't use `…`. But note how this uses additional layers beyond `tab_options()`.
