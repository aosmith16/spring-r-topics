...
```{r, echo = FALSE, out.width = "15%", fig.alt = "Logo for package reprex"}
...
.center[
```{r, echo = FALSE, out.width = "25%", fig.alt = "Logo for package datapasta"}
...
**You:** Are likely feeling overwhelmed right now because I just gave you a bunch of code that isn't relevant to the problem. `r emo::ji("laughing")`
...
Since my problem is with dodging it turns out I need a single `x` variable group to reproduce the problem. My example has only two rows of data.
]
...
Commonly used datasets include `mtcars` `r emo::ji("car")` and `iris` `r emo::ji("blossom")`, but there are a ton of other ones.  Run `data()` in your R Console to see all available datasets that come with R.

You can opt to use datasets that come with add-on packages, but remember that requires your potential helpers to have that package.
...
Here's how I can take `mtcars` and rewrite my code to reproduce the problem. I chose just two rows of the dataset that had the same value of `gear` but different values of `vs`.
...
There is much more you can do with **datapasta** then how we are using it here. Check out [How to Datapasta](https://milesmcbain.github.io/datapasta/articles/how-to-datapasta.html) for cool examples on copying and pasting data from online sources, etc.
...
**Note:** I found slight alignment issue with the output format from `dpasta()`, as shown above. When using the output in a question `Ctrl + I` (MacOS `Cmd + I`) will fix that right up. `r emo::ji("+1")`
...
Take note of all the extra columns. In keeping with making things minimal, I'll remove any column that isn't directly relevant to the problem prior to including it in my dataset.
...
**You:** Since you can now copy and paste the code into your R Console and immediately get it to run, you can jump right to helping me find solutions. `r emo::ji("tada")`
...
Now someone can copy your code and run it but, importantly, the can also see what the output looks like without having to run any code at all. 
...
Here's my reprex to copy again:
...
- Discuss the process and etiquette for reporting issues on GitHub repositories `r fontawesome::fa("github")`
