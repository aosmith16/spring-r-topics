...
It is standard to have a `README.md` file in your GitHub repository.  This file is for describing what project you are working on and why it is important. 

--

<br/><br/>
The README file is a Markdown document.  Markdown files render to HTML on GitHub so the information in the file will automatically be included in an easily-readable format on the main page of your repository.
...
Let's spend a few minutes looking at examples of README files.
...
<br/>
But plenty of people use repositories for other types of work. Here are examples for what others include in their README when working on a paper or talk/workshop instead of an R package.

- [Hill and Grolemund's repository for a workshop](https://github.com/rstudio-conf-2020/intro-to-ml-tidy)  
- [Christopherson et al.'s project repo](https://github.com/UW-MSDS-DATA-598-Reproducibility-WI20/Christopherson-Garcia-Todd-Winegarden-replication-project) demonstrates information you might want for a repository for an analysis    
...
A *merge conflict* is when the same lines of a file have been changed in two places, most often by different people. These are more common once you have collaborators.

I am going to have you create a merge conflict by changing a file on GitHub and in the local repository at the same time.  

--

We'll use GitKraken's `r icons::fontawesome("gitkraken")` merge conflict tool to address the conflict.
...
.center[
```{r, echo = FALSE, out.width = "75%", fig.alt = "Screenshot with the cursor hovering over the pencil shaped edit button when getting ready to edit the README file directly on GitHub"}
...
.center[
```{r, echo = FALSE, out.width = "68%", fig.alt = "Screenshot of committing the changes to the README file made directly on GitHub"}
...
Now open the Project for your local version of the repository in RStudio.

Open the README file and replace the current title and description with:
...
.center[
```{r, echo = FALSE, out.width = "80%", fig.alt = "Screenshot of merge conflict message after trying to pull changes GitHub when we've changed the same files in the local repository"}
...
### GitKraken merge `r icons::fontawesome("gitkraken")` conflict tool

We can choose all lines from one of the two versions. Here I show clicking on the "A" box to add all content from "A" to `Output`. I then remove the second line from `Output` by clicking next to that line. 
...
We will finish up today by going through the steps for making a *pull request* to propose changes to a repository.
...
Note that using pull requests is also a reasonable way to approach working with collaborators on a shared repository to avoid merge conflicts. 
...
.pull-right[
When this is finished a new RStudio Project will open from a new local directory for the forked repository.
...
Name your branch with your last name plus "prac". Here I named mine `muldoonprac`; names yours based on your own last name. `r emo::ji("wink")`
