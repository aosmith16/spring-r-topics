...
Today is a tasting menu of tools `r emo::ji("yum")`. We will briefly explore  
...
Adding interactive elements to static HTML elements allows for *pre-computed* interactive elements without needing a server. For more complex interactivity see package **shiny**.

--

*Before we begin:*

Make sure you saved `week08_interaction.Rmd` from the class website onto your computer. We will knitting this file and exploring the output.
...
- We'll be looking at the code and output from several **htmlwidgets**-packages that can be used to add interactive elements to HTML output.   

- Since we're just getting quick tastes of these packages you won't practice writing your own code.
...
We'll be loading additional packages as we go so it is clear which package we are using in each section.  
...
In R, the **plotly** package can be used to build stand-alone interactive graphics or to make a static plot interactive. `r emo::ji("astonished")`  
...
.pull-right[
Go to the knitted HTML output from `week08_interaction.Rmd` to see what this looks like. 

There is a table of contents at the top to help you navigate, as needed.
]
...
.pull-right[
```{r, echo = FALSE, fig.alt = "Interactive ggplot2 plot showing a scatter plot of the penguin data colored by species, with a hover tooltip that shows the x, y, and color data"}
gp1 = ggplotly(g1)
frameWidget(gp1)
```
]
...
<br/>
Let's put just the `y` and `x` values (in that order) in the tooltip, leaving out the `color` values.
]
...
.pull-right[
```{r, echo = FALSE, fig.alt = "Interactive ggplot2 plot showing a scatter plot of the penguin data colored by species, with a hover tooltip that shows the y data then the x data"}
...
You may have noted that you could select or *highlight* points in the scatterplot but then couldn't do much with them.  
...
.pull-left[
When we select points in the first plot, points from the same rows are selected in the second. This allows the user to explore more variables at one time.

<br/>
I found things were a little awkward for plots with legends. It helps a bit if the legend is removed from one of the plots using `plotly::style()`.
]

.pull-right[
```{r, echo = FALSE, fig.alt = "Two interactive ggplot2 scatter plots of the penguin data, allowing the user to pick an area in one and see the corresponding points in the other"}
gp3 = g2 %>%
  subplot(g3) %>%
  highlight(on = "plotly_selected",
            off = "plotly_deselect")
...
We can generate filters for each column in addition to the overall `Search` by adding the `filter` argument. We'll place the filters at the `top` of the table.
...
The package can render spatial objects created from **sf** or **sp** and also works with data.frames that contain latitude/longitude columns. Or center maps on a specific latitude/longitude.  
...
.pull-left[
We now use the `data` argument in `leaflet()`. **leaflet** automatically finds lat/long columns if named something recognizable (e.g., `lng`, `long`, `lat`, `latitude`).
...
<br/>
**Note:** There are `lng` and `lat` arguments in `addCircleMarkers()`. There are needed if the column names of the lat/long columns aren't recognized by `leaflet()` in the dataset.
]
...
<br/>
The `color` argument is mapped to `tjul` in `addCircleMarkers()`, wrapped in the `pal()` we made. 
...
I'm not getting into the colorQuantile() code too much here, but it is from **leaflet** and for mapping data to colors. This is a quick demonstration with no real details
...
.pull-left[
We'll look at male and female deaths from lung disease in the UK from 1974-1979 from **dygraphs** for this example.
...
There are a variety of dashboard layouts. The default is called a `fill` layout, and has room for three charts. One large chart is on the left and two smaller charts are on the right. We'll proceed with this layout, adding code to make the dashboard.
...
Note each of the three chart windows has been given a name.
]
...
You can see we load package **flexdashboard** by default.  Add **ggplot2**, **palmerpenguins**, **plotly**, **DT**, and **leaflet**.
...
- We'll discuss how to report issues on GitHub repositories  
- We will also practice making reproducible examples using package **reprex**  
- Make sure you have e a current version of **reprex** installed
