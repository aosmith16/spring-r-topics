...
Once the project is open and you can see the files, open `README.md` and add some text.

In the `Git` tab in RStudio, *stage* the README file by clicking on the box next to it. Staging tells Git you are getting ready to make the changed file part of the repository.
...
Then hit the `Commit` button. You are *committing* to make the edited files part of the repository.

---

## Update GitHub `r icons::fontawesome("github")` with local changes

Finally, `Push` the edited files to the remote repository.  
...
Once you start making a fair number of changes, entering your GitHub credentials every time you want to push becomes a pain.  I think the easiest thing to do is to add a SSH key in RStudio.
...
In the past I have had to write out the whole directory path in Windows. RStudio stores the key pair in your `Users` folder so the path I had to write out was `c:/Users/muldoona/.ssh/id_rsa` (my Users folder is named `muldoona` in this example). 
