...
We can control the column labels, cell alignment, column width and placement plus combine multiple columns with `cols_*()` functions.
...
***Code notes:***  
-Choose multiple `columns` to align with `vars()`  
...
-Combined values have space between them  
-By default the name of new column comes from first column listed  
]
...
<br/>
***Code notes:***  `USD` is the default currency symbol, although I wrote it out here  
]
...
The rows are what **gt** refers to as the *stub*. The stub is often a column of row labels that doesn't have (or need) a column label. We can also add grouping to put rows in different groups.
...
Watch out for missing values when summarizing. Also you likely noted non-summarized columns are given `---`. Change this with `missing_text` argument. 
...
Get rid of the line above and below the header as follows.
