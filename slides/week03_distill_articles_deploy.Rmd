...
meta() %>%
    ...
    meta_social(
        ...
        image = "https://raw.githubusercontent.com/aosmith16/spring-r-topics/main/slides/week03-share-card.png",
        image_alt = paste(
...
It is **distill** articles that we used to build a **distill** website last week and we'll continue with them today, adding additional content to your website.  
...
In the articles folder, not the root directory. Check twice.
...
If you have multiple authors, add a new `- name` entry to the YAML header. Watch your spacing!
...
If you don't want any author information, delete the whole `author:` section from the YAML header.
]
...
We don't currently have a URL for `article1` so I'll put in a filler URL to show what the output looks like. Note there are no spaces before `citation_url`.
...
After knitting, scroll down to the bottom of the output page. You now have a citation for the page and a BibTeX entry that readers can easily copy and paste to cite your work.
...
They won't have time do write all of this but could copy and paste.
...
.pull-right[
The BibTeX output is named using a *slug*, which it create automatically based on the given information. 

In this example the slug is `celis2019consequences`.  You can set this manually using the `slug` option in the YAML header, as well.
]
...
These particular citations are for R and R packages. Each entry has been given a name via a slug, which is the very first thing in the each entry.  
]
...
.pull-left[
For example, the entry for R is called `R-base`. This slug is how we'll refer to the citation in the text.
...
.pull-right[
To use the references in `article1.bib`, refer to it via `bibliography` in the YAML header. 

Here is an excerpt of the YAML header with this added:
...
Note I refer to `article1.bib` directly because it is in the same folder as `article1.Rmd`.
]

???

If need to go into more detail, the RMD document will by default look in the same folder as it is in for any documents. That's why we can refer to it directly, since it is in "articles" and so is article1.Rmd.
...
.pull-left[
Once you've defined your `.bib` file that contains the references, add citations using `@` with the `.bib` slug. To put citations in parentheses, use brackets.
...
*If you want a specific style for references, see instructions on how to change citation style in the [R Markdown Cookbook](https://bookdown.org/yihui/rmarkdown-cookbook/bibliography.html#changing-citation-style)*.


???

Point out the citation in parentheses in the body and the citation in the "References" section at the bottom.
...
There should be no spaces before `nocite:`. 

<br/>
I added this to the bottom of my current header in `article1.Rmd`.
]

--

.pull-right[
After  knitting, all references in the `.bib` file are included under `References`:
...
Process will be to add text to body of document and then knit to see the output.
...
Let's add some text to the body of `article2.Rmd` to see this in action. 
...
Add the text to the Rmd and knit.

Pic shows hovering and footnote as well as footnotes on bottom
...
Copy and paste the two asides into article2.Rmd to knit and see the result.
...
For a relatively simple website like the one we've made so far, one approach is to link to the article output from within one of our menu pages.

--

Open `about.Rmd`. Remember that the output of this is associated with my `Publications` menu on the website.

Here's the relevant `navbar` code from `_site.yml`:
...
Remind them that _site.yml defines all menus and the page associated with that menu so if they forget which menu goes with which document they can check.

---

## Adding articles as links

We'll add the articles to `about.Rmd` as if they contained information for the elements in a list of publications. This is one way to use a `Publications` menu.
...
Copy and paste this code and then we'll talk about the relative links. Should have done this more explicitly in week 2 when did pic in "images" folder.
...
We've been knitting individual files but now have made changes to the website so will build to see the whole thing.
...
This is an important aside. Some may not follow but should say anyway. 

Note use of package **here**.
...
Definitely watch the spacing when doing this.
...
We've set things up to use [GitHub Pages](https://pages.github.com/) for this. (*But also see [Netlify](https://www.netlify.com/).*)
...
Once you are in `Settings`, click on `Pages` near the bottom of the left sidebar. 

.center[
```{r, echo = FALSE, out.width = "75%", fig.alt = "Picture of GitHub Pages menu with sidebar"}
knitr::include_graphics("figs/week03_files/ghpages.png")
```
]

???

One up from bottom on left sidebar.
...
Refresh the page to check for changes. You are waiting (and waiting `r emo::ji("stopwatch")`) to see the message:
...
- Whenever you edit your website and push the changes to GitHub your website will automatically update. Be sure to wait a bit before checking if the updates took. `r emo::ji("wink")`

- If you want to take your website down, go back to `Settings > Pages`, choose `None` as the source, and `Save`. 
...
Seems like Netlify could make putting your name as part of the URL easier then GitHub pages, although will not cover that today.
