...
If you have multiple authors, add a new `- name` entry to the YAML header. 

I'm going to add myself as the second author. I include my affiliation but don't include any URL's so no links are added. Watch your spacing here!
...
.pull-left[
You cite using `@` with the `.bib` slug. To put citations in parentheses, use brackets.
...
<br/>
There should be no spaces before `nocite:`. I add this to the bottom of my current header in `article1.Rmd`.
]

--

.pull-right[
After  knitting, all references in the `.bib` file are included.
...
We'll add the articles as if they contained information for elements in a list of publications, hence we are working in the `Publications` menu.
