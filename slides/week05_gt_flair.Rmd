...
## Part 2: Adding flair `r emo::ji("boom")`: colors, images, & themes
...
-     Highlight specific values in columns    
-     Add a column of images      
...
To use the color scheme across all columns we set the `domain` using the range of values from the entire dataset. I use some helper functions from **dplyr** to get only the numeric columns when calculating the range.
...
***Code notes:***  
Note use of "magma" palette from viridis  
]
...
At the moment you cannot provide a named vector of colors in **gt** to map each category to each color like you can in **ggplot2**. 

Using `scales::col_factor()` and manually setting the `domain` plus using `ordered = TRUE` will allow us to assign colors to the correct factor levels.
...
```
***Code notes:***  
Note use of "viridis" palette from viridis  
]
...
-Chose rows of interest with an index   
]
...
Focus on the cell fill instead using `cell_fill()` for the `style` function. 
...
***Code notes:***  
-Used logical statement to get max value   
...
***Code notes*** This takes two `tab_styles()` and two logical statements
]
...
The process works the same way if we want to highlight across many columns. But every column chosen gets the same treatment. Here we work across all columns.
...
The work-horse for this is `text_transform()` in combination with `web_image()`.
...
We need to provide a function to the `fn` argument in `text_transform()`. We make an anonymous function to pass the image URL from each cell in the `hex` column to the `web_image()` function.
...
-Note also removed `hex` column label     
]
...
You will need to provide a path to the image. This could be as simple as providing the name of the image (e.g., `"myimage.png"`) or writing out a longer directory path (e.g., `"path/to/my/image/myimage.png"`).

--

Unlike with `web_image()`, this takes using a loop in `fn`. Here's some example pseudo-code:
...
A table theme is a function that sets the table options and overall table styling that you can apply to any table.  The function takes a **gt** table, applies styling elements to that table, and then returns the new table. 
...
***Code notes*** Adding `…` to both the table arguments and inside `tab_options()` allows the user to add in additional elements, as needed.
...
Because we used  `…` in the function we can pass additional elements to `tab_options()`.
...
Luckily other people have made themes for **gt** tables already. `r emo::ji("smile")` I'm guessing there will be more as the package ages.
...
Here's the theme that is based on the tables at FiveThirtyEight.
