...
It is just one option that allows us to build tables programmatically in R.
...
We need a couple few datasets to practice on. We will do all data manipulation steps now.
...
***Code notes:***  
-Choose columns to align using `vars()`  
...
You can manually define groups using the function `tab_row_group()` along with some sort of logical statement. 
...
Watch out for missing values when summarizing. You likely noted non-summarized columns are given `---`. Change this with `missing_text`. 
...
.foonote[*Columns do not have to be side-by-side to be grouped and then spanned*]
]
...
*Note numbering is in order of appearance in table, not from `tab_footnote()`*          
]
...
*Note use of `opt_align_table_header()` to left-align header*   
]
