...
In addition, you must have a `title` and `site: distill::distill_website` in the *YAML header*. The YAML header is the information at the top of the file sandwiched between two `---`.
...
- `_site.yml`: This file controls aspects of your website structure, including what is on the navigation bar.

--

- `about.Rmd`: The content of this file is currently associated with the `About` navigation bar button. Change the content of this file to change that page. (*Editing `about.Rmd` not covered today.*)
...
`href` must refer either to an HTML file in the `docs` folder or to a URL.
...
Let's change the YAML header filler info to our own before we knit. Delete `label`/`url` for social links you don't have and add others you want. Note my image is in the `images` folder.
...
Height is height of icons, which you can make larger or smaller
...
- Edit this file to change individual style elements of your website.
...
Run the following code in R to create a file called `theme.css` in your root directory.  It will open up in your `Source` pane once it has been created.
...
The **distill** website has three nice [example themes](https://rstudio.github.io/distill/website.html#example-themes) that can be a place to start. 
...
Save the result and build the website to see the changes. If you'd prefer something other look for your website, take time over the next week to find a theme you really like from the [distillery examples](https://jhelvy.github.io/distillery/showcase.html).
